...
class Battle:
    ...
    def get_battle_status(self):
        ...
        for i in self.battlers:
            ...
            embed.add_field(
                name=name, value=f"HP: `{bar}` `({remaining_hp}/{base_hp})`", inline=False)
    ...
    async def update_battlers_status(self):
        ...
        for i in range(len(self.battlers)):
            if self.battlers[i].hp > 0:
                ...
                if isinstance(self.battlers[i], Mob):
                    self.battlers[i].attack.stop()
    ...
    async def attack(self, attacker: int, skill):
        ...
        damage = Calc.calculate_damage(
            self.battlers[attacker], self.battlers[defender], skill)
...
class SkillButton(discord.ui.Button):
    def __init__(self, row, skill, player, battle_id):
        super().__init__(style=discord.ButtonStyle.primary,
                         label=skill["name"] if skill else "\u200b", row=row)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.disabled = True
        await interaction.response.z(view=self.view)
...
class PlayerAttackView(discord.ui.View):
    def __init__(self, player, battle_id):
        ...
        for i in player.equipped_skills:
            if cur >= 5:
                self.add_item(SkillButton(
                    2, GameData.get_skill(i), player, battle_id))
            else:
                self.add_item(SkillButton(
                    1, GameData.get_skill(i), player, battle_id))
            cur += 1
...
class BattleView(discord.ui.View):
    def __init__(self, battlers, battle_id):
        ...
        self.battlers = battlers
        self.players = {
            i.user.id: i for i in battlers if isinstance(i, Player)}
    ...
    async def open(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if user.id in self.players:
            embed = discord.Embed(title="Skill view",
                                  color=random.randint(0, 16777215))
...
class Skill:
    def __init__(self, name):
        ...
        skill = GameData.get_skill(self.skill_name)
        for key, value in skill.items():
            setattr(self, key, value)

skills = {}
skill_tree = {}
for skill in GameData.skills_data:
    skills[skill] = Skill(skill)
for skill in skills.keys():
    if skill in skill_tree:
        skill_tree[skills[skill].parent].append(skill)
    else:
        skill_tree[skill] = []

def display_skill_tree(cur_skill, indent):
    for skill in skills.keys():
        if skills[skill].parent == cur_skill:
            print("     " * indent, skill)
            display_skill_tree(skill, indent + 1)

for skill in skills.keys():
    if skills[skill].parent == None:
        print(skill)
